...
namespace ksr {
    ...
    /// Given a class template `template_` with type arguments only, determines whether `t` is an
    ...
    /// Given a class template `template_` with non-type (value) arguments only, determines whether
    /// `t` is an instantiation of that template.
